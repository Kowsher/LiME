...
#
# DoRA Formula: W' = m * (W + BA) / ||W + BA||_column
...
# -----------------------------
@torch.jit.script
def _dora_get_weight_norm(W: torch.Tensor, B: torch.Tensor, A: torch.Tensor, 
                          scaling: float, eps: float = 1e-8) -> torch.Tensor:
    """
    Compute column norms of (W + BA * scaling).
    W: (out_features, in_features)
    B: (out_features, rank)
    A: (rank, in_features)
    Returns: (out_features,)
    """
    combined = W + (B @ A) * scaling
    return combined.norm(p=2, dim=1).clamp_min(eps)


@torch.jit.script
def _dora_apply_magnitude(base_output: torch.Tensor, lora_output: torch.Tensor,
                          magnitude: torch.Tensor, column_norm: torch.Tensor) -> torch.Tensor:
    """
    Apply DoRA magnitude normalization.
    DoRA: out = m / ||W + BA|| * (Wx + BAx)
    
    base_output: (B, T, H) - output from frozen weights Wx
    lora_output: (B, T, H) - output from LoRA BAx * scaling
    magnitude: (H,) - learnable magnitude vector
    column_norm: (H,) - ||W + BA * scaling||_column
    
    Returns: (B, T, H)
    """
    # m / ||W + BA|| broadcast to (1, 1, H)
    mag_scale = (magnitude / column_norm).unsqueeze(0).unsqueeze(0)
    return (base_output + lora_output) * mag_scale


# =========================
# OPTIMIZED DoRALiMELinear
# =========================
class DoRALiMELinear(nn.Module, _StoresAux):
    """
    Fully optimized DoRA + MoE with ZERO routing parameters.
    
    DoRA (Weight-Decomposed Low-Rank Adaptation) decomposes the weight update
    into magnitude and direction components:
        W' = m * (W + BA) / ||W + BA||
    
    Where:
        - m: learnable magnitude vector (initialized to ||W||_column)
    ...
    def __init__(
        ...
        # DoRA magnitude vector - initialized to column norms of original weight
        with torch.no_grad():
            weight_norm = linear.weight.data.norm(p=2, dim=1)  # (H,)
            self.m = nn.Parameter(weight_norm.to(self.peft_dtype).clone())
    ...
    def _get_cached_params(self, compute_dtype: torch.dtype):
        ...
        m_cast = self.m.to(compute_dtype)
        ...
        return A_cast, B_cast, m_cast, LiMEs_cast, shared_cast, gamma_cast
    ...
    def forward(self, hidden_states: torch.Tensor, 
        ...
        A_cast, B_cast, m_cast, LiMEs_cast, shared_cast, gamma_cast = self._get_cached_params(compute_dtype)
        W = self.linear.weight.to(compute_dtype)
        ...
        H_out_base = F.linear(X, W, self.linear.bias.to(compute_dtype) if self.linear.bias is not None else None)
        
        # LoRA delta: X @ A^T @ B^T * scaling
        lora_out = (X @ A_cast.t()) @ B_cast.t() * self._scaling_val
        
        # Compute column norms of (W + BA * scaling)
        col_norm = _dora_get_weight_norm(W, B_cast, A_cast, self._scaling_val)
        
        # DoRA normalization scale
        mag_scale = (m_cast / col_norm).unsqueeze(0).unsqueeze(0)
        
        # Normalize SEPARATELY
        base_normalized = H_out_base * mag_scale  # m/||W+BA|| * Wx
        delta = lora_out * mag_scale               # m/||W+BA|| * BAx

        # === Routing ===
        H_out_full = base_normalized + delta  # For routing only
        H_slice = H_out_full[:, :, :E]
        delta_slice = delta[:, :, :E]
        
        # === N-GRAM ROUTING: subsample BEFORE computing probs ===
        if self.n_gram > 1:
            anchor_idx = _get_ngram_anchor_indices(T, self.n_gram, H_out_full.device)
        ...
        out = base_normalized + routed

        if attention_mask is not None:
            msk = attention_mask.bool()
            if msk.dim() == 2:
                msk = msk.unsqueeze(-1)
            out = torch.where(msk, out, base_normalized)
    ...
    def __repr__(self):
        ...
        return (f"DoRALiMELinear(in={self.linear.in_features}, "
                f"out={self.linear.out_features}, experts={self.num_experts}, "
                f"topk_mode={mode}{sparse_str}{ngram_str})")


# =========================
# OPTIMIZED DoRALiMEEmbedding  
# =========================
class DoRALiMEEmbedding(nn.Module, _StoresAux):
    """
    Fully optimized DoRA embedding version with sparse mode and n-gram support.
    
    For embeddings, we apply DoRA to the embedding matrix:
        E' = m * (E + BA) / ||E + BA||
    
    Where E is the embedding matrix (vocab_size, embed_dim).
    The magnitude m is per-embedding-dimension.
    ...
    def __init__(
        ...
        with torch.no_grad():
            weight_norm = embedding.weight.data.norm(p=2, dim=0)
            self.m = nn.Parameter(weight_norm.to(self.peft_dtype).clone())
    ...
    def forward(self, input_ids: torch.Tensor, 
        ...
        # Get base embedding
        E_base = self.embedding(input_ids)  # (B, T, H)
        compute_dtype = E_base.dtype
        H = E_base.shape[-1]
        ...
        A_cast, B_cast, m_cast, LiMEs_cast, shared_cast, gamma_cast = self._get_cached_params(compute_dtype)
        W = self.embedding.weight.to(compute_dtype)  # (V, H)

        # === DoRA Forward for Embeddings ===
        lora_weight = (B_cast @ A_cast) * self._scaling_val  # (V, H)
        combined_weight = W + lora_weight  # (V, H)
        col_norm = combined_weight.norm(p=2, dim=0).clamp_min(1e-8)  # (H,)
        
        mag_scale = m_cast / col_norm  # (H,)
        mag_scale_expanded = mag_scale.unsqueeze(0).unsqueeze(0)  # (1, 1, H)
        
        lora_out = F.embedding(input_ids, lora_weight)  # (B, T, H)
        
        base_normalized = E_base * mag_scale_expanded
        delta = lora_out * mag_scale_expanded

        # === Routing ===
        E_out_full = base_normalized + delta
        E_slice = E_out_full[:, :, :E]
        delta_slice = delta[:, :, :E]
        
        # === N-GRAM ROUTING: subsample BEFORE computing probs ===
        if self.n_gram > 1:
            anchor_idx = _get_ngram_anchor_indices(T, self.n_gram, E_out_full.device)
    ...
    def __repr__(self):
        ...
        sparse_str = ", sparse" if self.sparse_mode else ""
        ngram_str = f", n_gram={self.n_gram}" if self.n_gram > 1 else ""
        return (f"DoRALiMEEmbedding(vocab={self.embedding.num_embeddings}, "
...
def _replace_layers(
    ...
    for child_name, child in list(parent.named_children()):
        ...
        if isinstance(child, (DoRALiMELinear, DoRALiMEEmbedding)):
            continue

        if isinstance(child, nn.Linear) and wants(path, child):
            wrapped = DoRALiMELinear(
...
def apply_peft(
    ...
    if use_compile and hasattr(torch, 'compile'):
        for m in model.modules():
            if isinstance(m, (DoRALiMELinear, DoRALiMEEmbedding)):
...
def set_sparse_mode(model: nn.Module, sparse: bool):
    """Set sparse mode for all DoRA-LiME layers."""
    for m in model.modules():
        if isinstance(m, (DoRALiMELinear, DoRALiMEEmbedding)):
...
def set_n_gram(model: nn.Module, n_gram: int):
    """Set n_gram for all DoRA-LiME layers."""
    assert n_gram >= 1, "n_gram must be >= 1"
    for m in model.modules():
        if isinstance(m, (DoRALiMELinear, DoRALiMEEmbedding)):
            m.n_gram = n_gram
...
def set_peft_dtype(model: nn.Module, dtype: torch.dtype):
    for m in model.modules():
        if isinstance(m, (DoRALiMELinear, DoRALiMEEmbedding)):
            ...
            m.m.data = m.m.data.to(dtype)  # DoRA magnitude
            m.peft_dtype = dtype
...
def get_auto_topk_stats(model: nn.Module) -> Dict:
    ...
    for name, m in model.named_modules():
        if isinstance(m, (DoRALiMELinear, DoRALiMEEmbedding)) and m.auto_topk:
            if hasattr(m, "_last_usage") and m._last_usage is not None:
                usage = m._last_usage
...
def print_routing_summary(model: nn.Module):
    ...
    for name, m in model.named_modules():
        if isinstance(m, DoRALiMELinear):
            total_lora += m.A.numel() + m.B.numel()
            total_magnitude += m.m.numel()
            ...
            num_layers += 1
        elif isinstance(m, DoRALiMEEmbedding):
            total_lora += m.A.numel() + m.B.numel()
            total_magnitude += m.m.numel()
            total_LiMEs += m.LiMEs.numel()
            if m.LiME_shared is not None:
    ...
    print("=" * 60)
    print("DoRA-LiME (SPARSE MODE + N-GRAM ROUTING)")
    print("=" * 60)
    print(f"Wrapped layers:       {num_layers}")
    print(f"Routing parameters:   0")
    print(f"LoRA parameters:      {total_lora:,}")
    print(f"Magnitude parameters: {total_magnitude:,}")
    print(f"LiMEs parameters:     {total_LiMEs:,}")
    print(f"Total trainable:      {total_lora + total_magnitude + total_LiMEs:,}")
...
def compile_moe_layers(model: nn.Module, mode: str = "reduce-overhead"):
    ...
    print(f"Compiled {count} DoRA-MoE layers with mode='{mode}'")


# ---------- DoRA-specific utilities ----------
def get_magnitude_stats(model: nn.Module) -> Dict:
    """Get statistics about DoRA magnitude vectors."""
    stats = []
    for name, m in model.named_modules():
        if isinstance(m, (DoRALiMELinear, DoRALiMEEmbedding)):
            mag = m.m.detach().cpu()
    ...
    if not stats:
        return {"layers": [], "overall_mean": 0.0, "overall_std": 0.0}
    
    overall_mean = sum(s["mean"] for s in stats) / len(stats)
    overall_std = sum(s["std"] for s in stats) / len(stats)
...
def freeze_magnitude(model: nn.Module, freeze: bool = True):
    """Freeze or unfreeze DoRA magnitude vectors."""
    for m in model.modules():
        if isinstance(m, (DoRALiMELinear, DoRALiMEEmbedding)):
            m.m.requires_grad = not freeze


def reset_magnitude(model: nn.Module):
    """Reset magnitude vectors to original weight column norms."""
    for m in model.modules():
        if isinstance(m, DoRALiMELinear):
            with torch.no_grad():
                weight_norm = m.linear.weight.data.norm(p=2, dim=1)
                m.m.data.copy_(weight_norm.to(m.peft_dtype))
        elif isinstance(m, DoRALiMEEmbedding):
            with torch.no_grad():
                weight_norm = m.embedding.weight.data.norm(p=2, dim=0)
                m.m.data.copy_(weight_norm.to(m.peft_dtype))